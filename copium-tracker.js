 ...
 */
(function () {
    const STORAGE_KEY = "copium.estimates.v2";
    ...
    function setCountdownValues(ids, distanceMs) {
        ...
        const write = (id, val) => {
            ...
            if (el)
                el.innerHTML = `<span style="--value:${clamp(val)};"></span>`;
        };
        write(ids.days, days);
    ...
    window.createCopiumTracker = function createCopiumTracker(config) {
        const cfg = {
            ...
            countdownIds: {
                days:
                    (config.countdownIds && config.countdownIds.days) ||
                    "days",
                hours:
                    (config.countdownIds && config.countdownIds.hours) ||
                    "hours",
                minutes:
                    (config.countdownIds && config.countdownIds.minutes) ||
                    "minutes",
                seconds:
                    (config.countdownIds && config.countdownIds.seconds) ||
                    "seconds",
            },
